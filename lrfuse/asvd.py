...
from act_aware_utils import calib_input_distribution, calib_fisher_info, compress_model_asvd
from sensitivity import calib_sensitivity_ppl, calib_sensitivity_stable_rank, calib_sensitivity_ppl_rank_ratio
...
def main(args):
    ...
    if args.sensitivity_metric == "ppl":
        sensitivity = calib_sensitivity_ppl(model, calib_loader, args, args.use_cache, step=args.step ,act_aware=args.act_aware)
    ...
    selection_result = binary_search_truncation_rank(model, sensitivity, calib_loader, args)
    compress_model_asvd(model, selection_result, args)
